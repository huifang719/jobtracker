...
const JobBoard: React.FC = () => {
    ...
    jobs.length > 0 &&
      jobs.forEach((job: Job) => {
        ...
        const response = fetch(
          `https://jobtracker-typescript.onrender.com/api/save/${loggedInEmail}/${description}`
        )
          .then((res) => res.json())
          .then((res) => {
            if (res.error)
  ...
  const saveJob = async (index: number) => {
    ...
    const response = await fetch(
      "https://jobtracker-typescript.onrender.com/api/save",
      {
        method: "POST",
    ...
    return await setSaveStatus(
      saveStatus.map((status, i) => (i === index ? true : status))
    );
  };
  ...
  const deleteJob = async (index: number) => {
    ...
    const response = await fetch(
      `https://jobtracker-typescript.onrender.com/api/save/${description}`,
      {
        method: "DELETE",
      }
    ).then((res) => res.json());
    ...
    <>
      <Container className="d-block g-2">
        <IconContext.Provider
          value={{ color: "rgb(110,223,94)", size: "2rem" }}
        >
          {jobs.length > 0 &&
            jobs.map((job: Job, index: number) => (
              <Card key={index} className="mb-2">
                ...
                <Card.Body>
                  ...
                  <Button
                    style={{
                      backgroundColor: "rgb(110,223,94)",
                      border: "none",
                    }}
                    href={job.url}
                  >
